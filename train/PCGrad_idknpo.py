...
You have two objectives:
  ...
  (B) forget objective (label=1): DPO-style preference aligning model to output an "I don't know" response
      chosen   = idk_text
      rejected = toxic generation from data

We apply PCGrad to combine the two objectives each micro-step:
  - compute grads for retain objective
  - compute grads for forget objective
  - project away conflicting components (PCGrad)
  - accumulate projected grads across grad_accum, then optimizer step

Run:
  torchrun --nproc_per_node=2 train_idkdpo_pcgrad_fsdp.py \
    --data_path data.pkl \
    ...
    --batch_size_retain 2 --batch_size_forget 2 --grad_accum 8

Notes:
  - Uses length-based attention_mask (safe even when pad_token_id == eos_token_id).
  - DPO uses mean logp per supervised token (length-normalized).
  - PCGrad projection scalars are computed globally across ranks (correct with FSDP sharding).
  - fp16 uses GradScaler; PCGrad is scale-invariant so we operate on scaled grads and unscale once at step.
"""
...
class TokenConcatHelper:
    ...
    def length_mask(lengths: List[int], max_len: int) -> torch.Tensor:
        ar = torch.arange(max_len, dtype=torch.long).unsqueeze(0)
        ln = torch.tensor(lengths, dtype=torch.long).unsqueeze(1)
        return (ar < ln).long()
...
def sequence_logp_mean_from_labels(logits: torch.Tensor, labels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    gather_labels = shift_labels.masked_fill(~mask, 0).unsqueeze(-1)
    token_logp = log_probs.gather(-1, gather_labels).squeeze(-1)
...
def snapshot_param_grads(params: List[torch.nn.Parameter]) -> List[torch.Tensor]:
    """
    Returns a list of grad tensors aligned with params.
    Grad tensors are sharded under FSDP (correct).
    """
    out: List[torch.Tensor] = []
    for p in params:
        if p.grad is None:
            out.append(torch.zeros_like(p, device=p.device))
        else:
...
def global_dot(grads_a: List[torch.Tensor], grads_b: List[torch.Tensor]) -> torch.Tensor:
    ...
    """
    dev = grads_a[0].device if len(grads_a) > 0 else torch.device("cuda")
    s = torch.zeros((), device=dev, dtype=torch.float32)
    for ga, gb in zip(grads_a, grads_b):
        if ga is None or gb is None:
            continue
        s = s + (ga.float() * gb.float()).sum()
    allreduce_sum_(s)
    return s
...
def pcgrad_project(
    ...
    """
    PCGrad:
      - grads_by_task: list of task gradients, each is list aligned with params
    Returns:
      - merged grads list aligned with params
      - stats dict (conflict count, dot, norms)
    """
    num_tasks = len(grads_by_task)
    if num_tasks == 1:
        return grads_by_task[0], {"pcgrad_conflicts": 0.0}

    device = grads_by_task[0][0].device
    orig = grads_by_task
    pc = [[g.clone() for g in task_grads] for task_grads in orig]

    perm = synced_task_perm(num_tasks, seed=seed, device=device)
    conflicts = 0.0
    ...
    for i in perm:
        ...
        perm2 = synced_task_perm(num_tasks, seed=seed + 1337 + i, device=device)
        for j in perm2:
            if j == i:
                continue
            dot = global_dot(pc[i], orig[j])  # current pc[i] vs original grad[j]
            if dot.item() < 0:
                denom = global_norm2(orig[j]).clamp_min(eps)
                coef = (dot / denom).to(dtype=torch.float32)
                # pc[i] -= coef * orig[j]
                for k in range(len(pc[i])):
                    pc[i][k].add_(orig[j][k], alpha=-coef.item())
                conflicts += 1.0

    merged = [torch.zeros_like(pc[0][k]) for k in range(len(pc[0]))]
    for t in range(num_tasks):
        for k in range(len(merged)):
            merged[k].add_(pc[t][k])

    stats = {
        "pcgrad_conflicts": float(conflicts),
        "pcgrad_dot_retain_forget": float(dot01),
        "pcgrad_norm_retain": float(n0),
        "pcgrad_norm_forget": float(n1),
    }
    return merged, stats
...
def parse_args():
    ...
    p.add_argument("--idk_lm_coef", type=float, default=0.05)

    # PCGrad
    p.add_argument("--pcgrad_eps", type=float, default=1e-12)
    ...
    # logging/saving
    p.add_argument("--wandb_project", type=str, default="idkdpo-pcgrad-unlearning")
...
def main():
    ...
    if tok.pad_token_id is None:
        tok.pad_token = tok.eos_token  # safe due to length masks
    ...
    if len(retain_rows) == 0:
        raise ValueError("No label=0 (retain) rows found.")
    ...
    micro_steps_per_epoch = max(len(forget_loader), len(retain_loader))
    steps_per_epoch = micro_steps_per_epoch // max(1, args.grad_accum)
    ...
    # trainable param list for grad snapshots / accumulation
    trainable_params = [p for p in model.parameters() if p.requires_grad]

    # grad accumulation buffers (same dtype/device as grads)
    ...
    for epoch in range(args.epochs):
        ...
        for _ in range(micro_steps_per_epoch):
            ...
            # --------------------
            zero_param_grads(trainable_params)
            with autocast_ctx():
                out_ret = model(
            ...
            if args.fp16:
                scaler.scale(retain_obj).backward()
            else:
                retain_obj.backward()

            grads_retain = snapshot_param_grads(trainable_params)

            # --------------------
            # 2) grads for forget objective (DPO preferring IDK)
            # --------------------
            zero_param_grads(trainable_params)
            with autocast_ctx():
                out_c = model(
            ...
            if args.fp16:
                scaler.scale(forget_obj).backward()
            else:
                forget_obj.backward()

            grads_forget = snapshot_param_grads(trainable_params)

            # --------------------
            # 3) PCGrad merge (retain vs forget)
            # --------------------
            pc_seed = args.seed + (epoch * 10_000_000) + (opt_step * 10_000) + micro
            merged_grads, pc_stats = pcgrad_project(
                grads_by_task=[grads_retain, grads_forget],
                seed=pc_seed,
                eps=args.pcgrad_eps,
            )

            # accumulate projected grads
            for k in range(len(accum_grads)):
                accum_grads[k].add_(merged_grads[k])
            ...
            # --------------------
            if do_step:
                # install accumulated grads into params
                for p_, g_ in zip(trainable_params, accum_grads):
                    p_.grad = g_

                if args.fp16:
                    # unscale once (PCGrad is scale-invariant; we kept scaled grads during accumulation)
